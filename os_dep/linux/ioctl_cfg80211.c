...
static void cfg80211_rtw_update_mgmt_frame_registrations(
	struct wiphy *wiphy,
	struct wireless_dev *wdev,
...
#ifdef CONFIG_DEBUG_CFG80211
	RTW_INFO(
		FUNC_ADPT_FMT" old reg:%x, new reg:%x\n",
		FUNC_ADPT_ARG(adapter),
		pwdev_priv->report_mgmt,
		upd->interface_stypes
	);
#endif

	// Driver only supports the AUTH flag (based on implementation in ELSE case)
	pwdev_priv->report_mgmt = (upd->interface_stypes & IEEE80211_STYPE_AUTH) >> 4;
